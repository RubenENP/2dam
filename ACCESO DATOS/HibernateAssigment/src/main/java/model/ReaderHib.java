...
@NamedQueries({ @NamedQuery(name = "HQL_GET_ALL_READERS", query = "from ReaderHib "),
        ...
        @NamedQuery(name = "HQL_GET_ALL_READERS_OF_NEWSPAPER", query = "SELECT s.reader from Subscription s WHERE s.newspaper.nameNewspaper = :name AND s.cancellationDate > :date")})
